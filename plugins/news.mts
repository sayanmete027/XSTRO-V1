...
Module(
    ...
    async (message: Message) => {
        ...
        const data = news.map((article, index) => `${index + 1}. ${article.title}\n${article.description || "No description available."}\n${article.link}`).join("\n\n");
...
Module(
    ...
    async (message: Message) => {
        ...
        const news = res.map((item: any, index) => `${index + 1}. ${item.title}\n${item.description || "No description available."}\n[Read more](${item.url})\n`).join("\n");
...
Module(
    ...
    async (message: Message) => {
        ...
        const data = res.map((article) => `Title: ${article.title}\nAuthor: ${article.author}\nUrl: ${article.url}\n\n`);
