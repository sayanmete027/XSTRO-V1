...
(0, src_1.Module)({
    ...
    const allChats = await (0, src_1.getChatSummary)();
    const groupChats = allChats.filter((chat) => (0, resources_1.isJidGroup)(chat.jid));
    if (!groupChats)
        return msg.reply('No Groups found!');
    const data = (await Promise.all(groupChats.map(async (data) => {
        var _a;
        const subject = (_a = (await (0, src_1.groupMetadata)(data.jid))) === null || _a === void 0 ? void 0 : _a.subject;
...
(0, src_1.Module)({
    ...
    if (groupData.length === 0)
        return await msg.send('No active members found.');
...
(0, src_1.Module)({
    ...
    if (groupData.length === 0)
        return await msg.reply('No inactive members found.');
