...
async function saveInputFile(buffer) {
  const fileType = await (0, index_1.FileTypeFromBuffer)(buffer);
...
const GIFBufferToVideoBuffer = async image => {
  ...
  try {
    ...
    if (existsSync(mp4Path)) fs_1.default.unlinkSync(mp4Path);
    if (existsSync(gifPath)) fs_1.default.unlinkSync(gifPath);
    throw error;
  }
};
...
async function audioToBlackVideo(input) {
  ...
  try {
    ...
    if (existsSync(video)) fs_1.default.unlinkSync(video);
    if (existsSync(inputFile)) fs_1.default.unlinkSync(inputFile);
    throw error;
  }
}
async function flipMedia(input, direction) {
  const inputFile = await saveInputFile(input);
  const fileType = await (0, index_1.FileTypeFromBuffer)(input);
  ...
  try {
    ...
    if (existsSync(outputFile)) fs_1.default.unlinkSync(outputFile);
    if (existsSync(inputFile)) fs_1.default.unlinkSync(inputFile);
    throw error;
  }
}
async function webpToImage(input) {
  ...
  try {
    ...
    if (existsSync(outputImage)) fs_1.default.unlinkSync(outputImage);
    if (existsSync(inputFile)) fs_1.default.unlinkSync(inputFile);
    throw error;
  }
}
async function convertToMp3(input) {
  ...
  try {
    ...
    if (existsSync(outputAudio)) fs_1.default.unlinkSync(outputAudio);
    if (existsSync(inputFile)) fs_1.default.unlinkSync(inputFile);
    throw error;
  }
}
...
async function toVideo(input) {
  ...
  try {
    ...
    if (existsSync(outputVideo)) fs_1.default.unlinkSync(outputVideo);
    if (existsSync(inputFile)) fs_1.default.unlinkSync(inputFile);
    throw error;
  }
}
const cropToCircle = async input => {
  try {
    ...
    const croppedImage = await image.composite([{
      input: circleMask,
      blend: 'dest-in'
    }]).toFormat('webp', {
      quality: 50
...
async function resizeImage(imageBuffer, width, height) {
  ...
  return (0, sharp_1.default)(imageBuffer).resize(width, height, {
    fit: 'inside',
...
const isAnimatedWebp = filePath => {
  return new Promise((resolve, reject) => {
    (0, sharp_1.default)(filePath).metadata().then(metadata => {
...
async function convertWebPFile(media) {
  try {
    ...
    for (let i = 0; i < metadata.pages; i++) {
      const frame = await (0, sharp_1.default)(media, {
        page: i
      }).jpeg({
        quality: 90
      }).toBuffer();
    ...
    await fs_1.default.promises.rm(tempDir, {
      recursive: true
    });
    return buffer;
  } catch (error) {
    try {
      await fs_1.default.promises.rm('temp_frames', {
        recursive: true,
        force: true
      });
    } catch (cleanupError) {
...
async function imageToWebp(media) {
  ...
  try {
    ...
    if (fs_1.default.existsSync(tmpFileIn)) fs_1.default.unlinkSync(tmpFileIn);
    if (fs_1.default.existsSync(tmpFileOut)) fs_1.default.unlinkSync(tmpFileOut);
...
const createSticker = async (buffer, author, packname) => {
  const mime = await (0, index_1.getMimeType)(buffer);
  let res;
  const options = {
    packname: packname || index_2.LANG.STICKER_META.split(';')[0],
    author: author || index_2.LANG.STICKER_META.split(';')[1]
  };
  try {
