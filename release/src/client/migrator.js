...
async function SessionMigrator(Sessionfolder, SessionDataBasePath) {
    try {
        const sId = await (0, src_1.getSessionId)();
        ...
        async function readSessionFiles() {
            ...
            await Promise.all(syncKeyFiles.map(async (file) => {
                ...
                if (dynamicPart.endsWith('.json'))
                    dynamicPart = dynamicPart.slice(0, -5);
        ...
        if (sessionData.creds) {
            const credsValue = typeof sessionData.creds === 'object'
        ...
        await (0, src_1.setSessionId)(config_1.default.SESSION_ID);
        return sessionData;
    }
    catch (_a) {
        console.log('No Migration');
