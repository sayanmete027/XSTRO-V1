...
const saveContact = async (jid, name) => {
    if (!jid || !name || (0, resources_1.isJidGroup)(jid) || (0, resources_1.isJidBroadcast)(jid) || (0, resources_1.isJidNewsletter)(jid))
        return;
    const db = await (0, database_1.getDb)();
    await initStoreDb();
...
const saveMessage = async (message) => {
    ...
    if ((message === null || message === void 0 ? void 0 : message.sender) && (0, resources_1.isJidGroup)(jid)) {
...
const getName = async (jid) => {
    if ((0, resources_1.isJidGroup)(jid) || (0, resources_1.isJidBroadcast)(jid) || (0, resources_1.isJidNewsletter)(jid))
        return null;
    const db = await (0, database_1.getDb)();
    await initStoreDb();
...
const getInactiveGroupMembers = async (jid) => {
    if (!(0, resources_1.isJidGroup)(jid))
        return [];
    const groupdata = await (0, src_1.groupMetadata)(jid);
    if (!groupdata)
        return [];
    const db = await (0, database_1.getDb)();
    await initStoreDb();
